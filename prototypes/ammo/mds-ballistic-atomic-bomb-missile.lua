...
khaoslib_ammo.copy("rocket", "mds-ballistic-atomic-bomb-missile")
  :set {
    order = "d[rocket-launcher]-e[mds]-d[atomic-bomb]",
      ...
      :replace_action(function(action)
        ...
        return khaoslib_trigger_item:load(action)
          :replace_action_delivery(function(delivery)
            ...
            type = "artillery",
            projectile = "mds-ballistic-artillery-projectile-atomic-bomb",
            starting_speed = ballistic_constants.speed,
            direction_deviation = ballistic_constants.direction_deviation * 4,
            range_deviation = ballistic_constants.range_deviation * 4,
            source_effects = {
              {
                type = "script",
