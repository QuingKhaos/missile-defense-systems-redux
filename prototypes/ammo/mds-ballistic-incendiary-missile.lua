...
if mods["chemical-rocket"] then
  khaoslib_ammo.copy("rocket", "mds-ballistic-incendiary-missile")
    :set {
      order = "d[rocket-launcher]-e[mds]-b[explosive]-a",
      ammo_category = "mds-ballistic-missile",
      magazine_size = 10,
    }
    :set_icons(khaosbash.load_icons("__missile-defense-systems-redux__/graphics/icons/mds-ballistic-missile", util.color("fc9100")))
        ...
        :replace_action(function(action)
          ...
          return khaoslib_trigger_item:load(action)
            :replace_action_delivery(function(delivery)
              ...
              type = "artillery",
              projectile = "mds-ballistic-artillery-projectile-incendiary",
