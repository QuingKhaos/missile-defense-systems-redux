...
if mods["chemical-rocket"] then
  khaoslib_artillery_projectile.copy("artillery-projectile", "mds-ballistic-artillery-projectile-incendiary")
    :set {
      reveal_map = false,
      rotatable = true,
    ...
    :unset("shadow")
    :set {
      picture = require("__base__.prototypes.entity.rocket-projectile-pictures").animation(util.color("fc9100")),
      shadow = require("__base__.prototypes.entity.rocket-projectile-pictures").shadow,
    }
    :unset("action")
    :set {action = khaoslib_projectile.get("incendiary-rocket").action}
    :commit()
end
